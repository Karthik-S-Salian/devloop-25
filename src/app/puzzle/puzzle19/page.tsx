  ...
  useEffect(() => {
    ...
    const updateBallPosition = (): void => {
      if (gyroscope.current) {
        ...
        ballPos.current.x = Math.max(ballRadius, Math.min(300 - ballRadius, ballPos.current.x + xMovement));
        ballPos.current.y = Math.max(ballRadius, Math.min(300 - ballRadius, ballPos.current.y - yMovement)); // Inverted Y-axis
      }
    };
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div>
          ...
          <Button onClick={stopGame} disabled={!isPlaying} variant="outline" className="ml-2">
            Stop Game
          </Button>
        </div>
      </CardContent>
